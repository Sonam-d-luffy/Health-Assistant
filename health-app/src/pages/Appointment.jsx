...
const Appointment = () => {
  ...
  useEffect(() => {
    ...
    setUser({email , name})
      ...
      try {
        const res = await axios.get(`http://localhost:4000/api/hospitals/${hospitalId}/departments`);
    ...
    if (formdata.department) {
      const fetchDoctors = async () => {
        try {
          const res = await axios.get(`http://localhost:4000/api/hospitals/${hospitalId}/doctors/${formdata.department}`);
  ...
  const handleSubmit = async(e) => {
    ...
    try {
     ...
     const res = await axios.post('http://localhost:4000/api/appointments/book' , {
        ...formdata,
