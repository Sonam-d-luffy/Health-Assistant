...
const HospitalLogin = () => {
 const [name , setName] = useState('')
  ...
  const handleLogin = async(e) => {
    e.preventDefault()
    try {
      const endpoint = login ? 'http://localhost:4000/api/hospitalAuth/login' : 'http://localhost:4000/api/hospitalAuth/signup'
      const formData = login ? {email , password} : {name , email , password}
      const res = await axios.post(endpoint , formData)
   
      localStorage.setItem('token' , res.data.token)
      localStorage.setItem('hospitalEmail', res.data.userExist?.email || res.data.user?.email);
      localStorage.setItem('hospitalId', res.data.userExist?._id || res.data.user?._id);
